...
def get_boxes(height, width, x, y, \
    ...
    if not (height > min_height*2 + spacing \
        ...
        boxes.append((x,y,width,height, 5))
        return boxes
    ...
    if (height < 3*min_height or width < 3*min_width) \
        ...
        boxes.append((x,y,width,height,np.random.randint(2)))
        return boxes
    ...
    # else we are gonna divide the boxes into half
        ...
        if direction == 'row':
            break_point = np.random.randint(x + min_width, x + width - min_width - spacing)
            box1 = get_boxes(height,break_point-x,x,y)
            box2 = get_boxes(height,x+width-spacing-break_point,break_point+spacing,y)
            return box1 + box2
        else: 
            break_point = np.random.randint(y + min_height, y + height - min_height - spacing)
            box1 = get_boxes(break_point-y,width,x,y)
            box2 = get_boxes(y+height-spacing-break_point,width,x,break_point+spacing)
            return box1 + box2
...
def render_from_layout(args, bboxes, im_height, im_width):
    ...
    for (x, y, w, h, c) in bboxes:
        ...
        out_img, para, _, text = fill_text(
            args, img, (x, y, w, h), font, font_title, fontsize, render_type=c_name)
...
def main(args):
    ...
    for idx, im_name in enumerate(range(args.resume_from, args.resume_from+args.repeat)):
        t               = time.time()
        global_height   = np.random.randint(1500,2500)
        global_width    = np.random.randint(1500,2500)
        bboxes          = get_boxes(global_height-200,global_width-200,100,100,
                                    spacing=args.col_spacing)
        
        im, para        = render_from_layout(args, bboxes, global_height, global_width)
        fin             = get_img('assets/bg', 1.0).resize((global_width, global_height))
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description="Argument to pass to the generator")
    parser.add_argument('--num_core',default=16,type=int,help='number of cores to be used for generation')
