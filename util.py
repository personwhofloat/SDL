...
def render_page(args, img):
    ...
    args.prob_image = 0.5/num_col
    args.prob_table = 0.6/num_col
    
    
    start = np.random.rand() * 0.1+0.005
    ...
    if num_col == 1:
        ...
        for i in range(num_comp-1):
            out_img, para, _, texts = fill_text(args, out_img, (int(xs[0]),y0,int(xs[1]-xs[0]),hs[i]),font,font_title,fontsize)
        ...
        for _ in range(num_col):
            ...
            hs *= 0.85*img.size[1]
            hs = hs.astype('int')
            y0 = int(0.05*img.size[1])
            
            for i in range(num_comp):
                out_img, para, _, texts = fill_text(args, out_img, (int(x0),y0,int((xs[1]-xs[0])/num_col-args.col_spacing/2),hs[i]),font,font_title,fontsize)
