...
def get_img(path,threshold=0.7):
    ...
    if text_type == 'number':
        ...
        while True:
            ...
            if np.random.exponential(args.period_prob) > 2*args.period_prob:
                word += '.'
            if start and title:
                word = f'{np.random.randint(1,7)}.'+word
            if start and starting:
            ...
            if w > max_width: 
                ...
                if not in_table:
                    if under_fit > 0:
                        ...
                        for i in indexes:
                            text[i] += np.random.choice(['i','r','.','l',':',';','I','1','!'])
...
def fill_text(args, img, bbox, font, font_title, fontsize, text_type = 'text', in_table = False, render_type='all', title=False):
    '''
    ...
    if bbox[3]>100 and bbox[3]/bbox[2] > 0.3 and render_type=='all':
        ...
        r = np.random.rand()
        if r < args.prob_image:
            return render_figure(args, out_img, bbox, fontsize)
        elif r > 1 - args.prob_table:
    ...
    if render_type == 'figure':
        # out_img = np.array(img)
        return render_figure(args,np.array(img),bbox,fontsize)
...
def render_figure(args,img,bbox,fontsize):
    s_img = get_img(args.doc_images,threshold=1.0).convert('RGBA').resize((bbox[2],bbox[3]))
    s_img = np.array(s_img)
    img[bbox[1]:bbox[1]+bbox[3],bbox[0]:bbox[0]+bbox[2],:] += s_img
    para = {'component': 'figure', 'bbox':(int(bbox[0]), int(bbox[1]), int(bbox[0])+math.ceil(bbox[2]),int(bbox[1])+math.ceil(bbox[3]))}
    words = []
    texts = []
...
def render_table(args, img, bbox, font_text, font_title, fontsize, padding=5):
    ...
    for i in range(num_row+1):
        if i == 0:
            ...
            if np.random.rand() > 0.8 and i < num_row:
                draw.rectangle((x, int(y0), x+w,
...
def render_title(args, img, bbox, font, font_title, fontsize):
    ...
    font_title = get_font(args, fontsize+2,bold=True)
    out_img, para, word, text = fill_text(args, out_img, (x,y,w*(0.3+np.random.rand()*0.7),(fontsize+2)*1.1),font_title,font_title,fontsize+2, title=True)
    para['component'] = 'title'
    paras.append(para)
    words = word
    texts = text
    y += (1+args.spacing)*fontsize
...
if __name__ == '__main__':
    ...
    out_img, _ = render_title(args, out_img, bbox, font,font_title,fontsize)
