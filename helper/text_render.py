...
def render_text_line(args, img, xy, max_width, font, text_type='text', starting=False, ending=False, in_table=False, title=False):
    ...
    if text_type == 'number':
        ...
        while True:
            ...
            if start and title and np.random.rand() < 0.6:
...
def render_title(args, img, bbox, font, font_title, fontsize):
    ...
    new_fs = fontsize + np.random.choice([*range(1,5),min(10,int(h/(1+args.spacing)))])
    font_title = get_font(args, new_fs,bold=True)
    color = args.text_color
    if np.random.rand() < 0.2:
        args.text_color = (*np.random.randint(0,88,3).tolist(),255)
    if np.random.rand() < 0.5:
        out_img, para, word, text = fill_text(args, out_img, (x,y,w*(0.3+np.random.rand()*0.7),new_fs),font_title,font_title,fontsize+2, title=True)
    else:
        width = w*(0.3+np.random.rand()*0.7)
        out_img, para, word, text = fill_text(args, out_img, (x+(w-width)/2,y,width,new_fs),font_title,font_title,fontsize+2, title=True)
    args.text_color = color
    para['component'] = 'title'
    paras.append(para)
    words = word
    texts = text
    y += (1+args.spacing)*new_fs
